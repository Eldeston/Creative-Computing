...
def openFile() :
    ...
    filePath = "Year 2 - Semester 1/3 - Advanced Programming/Activities/Activity 06/beemovie.txt"

    with open(filePath, "r") as fileHandler :
        contents = fileHandler.readline()
        # contents = open(filePath, "r").readline()

        # contents = fileHandler.readlines()
        # contents = random.choice(contents).strip()

    with open(filePath, "w") as fileHandler :
        fileHandler.write("Skibidi " + contents)
        # open(filePath, "w").write("Skibidi " + contents)
