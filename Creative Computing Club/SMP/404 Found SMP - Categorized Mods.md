# DEFAULT MODS
## Core Default Mods
Core mods that are required to join the server.

| Core Mods                                              | Status |
| ------------------------------------------------------ | ------ |
| Accessories                                            |        |
| Accessories Compatibility Layer                        |        |
| Accessorify                                            |        |
| Biomes O Plenty                                        |        |
| Chef's Delight - Farmer's Delight Villagers            |        |
| Cobbleloots: Loot Balls and More!                      |        |
| Cobblemon                                              |        |
| Cobblemon Add-on: Mega Showdown Ores                   |        |
| Cobblemon Fight or Flight Reborn                       |        |
| Cobblemon Integrations                                 |        |
| Cobblemon Journey Mounts                               |        |
| Cobblemon Leaderboards                                 |        |
| Cobblemon Pokemon Badges                               |        |
| Cobblemon Pokenav                                      |        |
| Cobblemon Raid Dens                                    |        |
| Cobblemon Size Variation                               |        |
| Cobblemon: Legends Untold                              |        |
| Cobblemon: Mega Showdown                               |        |
| Complete Cobblemon Collection w/ Legendary Spawns      |        |
| Corpse                                                 |        |
| Create                                                 |        |
| Create: Central Kitchen                                |        |
| Create: Copycats+                                      |        |
| Create: Design n' Decor                                |        |
| Create: Dragons Plus                                   |        |
| Create: Easy Structures                                |        |
| Create: Encapsulated                                   |        |
| Create: Enchantment Industry                           |        |
| Create: Let The Adventure Begin                        |        |
| Extra Move Animations (Cobblemon)                      |        |
| Farmer’s Delight                                       |        |
| Lootr                                                  |        |
| Lootrmon: Lootr & Cobblemon Gilded Chest Compatibility |        |
| Navas ZA Megas                                         |        |
| Open Parties and Claims                                |        |
| PVP Flagging                                           |        |
| Radical Cobblemon Trainers                             |        |
| Sophisticated Backpacks                                |        |
| Sophisticated Backpacks Create Integration             |        |
| Sophisticated Core                                     |        |
| Spice of Life Onion                                    |        |
| Tomtaru's Cobblemon and Farmer's Delight Tweaks        |        |
| Waystones                                              |        |

## Client Only Default Mods
Optional mods to enhance gameplay.

| Client Mods              | Status |
| ------------------------ | ------ |
| AmbientSounds            |        |
| BetterF3                 |        |
| Chat Heads               |        |
| Cobblemon Battle Extras  |        |
| Controlling              |        |
| Cubes Without Borders    |        |
| Discord & Chat Images    |        |
| Distant Horizons         |        |
| EMI                      |        |
| Fancy World Animations   |        |
| FancyMenu                |        |
| Freecam                  |        |
| Iris                     |        |
| Jade                     |        |
| Jade Addons              |        |
| Just Zoom                |        |
| LambDynamicLights        |        |
| MoreCobblemonTweaks      |        |
| Particular Reforged      |        |
| Raised                   |        |
| Reese’s Sodium Options   |        |
| Reforgedplaymod          |        |
| RenderScale              |        |
| Screenshot Viewer        |        |
| Searchables              |        |
| Simple Voice Chat        |        |
| Sound Physics Remastered |        |
| Wakes Reforged           |        |
| Xaero’s Minimap          |        |
| Xaero’s World Map        |        |
| XaeroPlus                |        |

## Server Only Default Mods
Optional mods to match singleplayer gameplay with server gameplay.

| Server Mods                                    | Status |
| ---------------------------------------------- | ------ |
| Berry Harvester                                |        |
| Cobblemon Capture XP                           |        |
| Cobblemon Spawn Notification                   |        |
| Cobblemon: Extra Structures                    |        |
| Cobblemon: Infinite Riding Stamina [1.7]       |        |
| Dungeons and Taverns                           |        |
| Dungeons and Taverns Ancient City Overhaul     |        |
| Dungeons and Taverns Desert Temple Overhaul    |        |
| Dungeons and Taverns Jungle Temple Overhaul    |        |
| Dungeons and Taverns Nether Fortress Overhaul  |        |
| Dungeons and Taverns Ocean Monument Overhaul   |        |
| Dungeons and Taverns Pillager Outpost Overhaul |        |
| Dungeons and Taverns Stronghold Overhaul       |        |
| Dungeons and Taverns Swamp Hut Overhaul        |        |
| Dungeons and Taverns Woodland Mansion Overhaul |        |
| Skin Restorer                                  |        |
| Towns and Towers                               |        |

# OPTIMIZATION MODS
## Core Optimization Mods
Optimization mods that are required on both client & server.

| Core Mods              | Status |
| ---------------------- | ------ |
| Almost Unified         |        |
| Alternate Current      |        |
| AttributeFix           |        |
| Autochef's Delight     |        |
| Fast Item Frames       |        |
| Fast Paintings         |        |
| Not Enough Recipe Book |        |

## Client Only Optimization Mods
Optimization mods that are required on client only. Cannot be installed on server because of redundancy.

| Client Mods                                | Status |
| ------------------------------------------ | ------ |
| BadOptimizations                           |        |
| Better Biome Reblend                       |        |
| Colorwheel                                 |        |
| Colorwheel Patcher                         |        |
| CreateBetterFps                            |        |
| Distraction Free Recipes (EMI / REI / JEI) |        |
| Dynamic FPS                                |        |
| Enhanced Block Entities NEOFORGED          |        |
| Entity Culling                             |        |
| ImmediatelyFast                            |        |
| More Culling                               |        |
| Particle Core                              |        |
| Sodium                                     |        |
| Sodium Extra                               |        |

## Server Only Optimization Mods
Optimization mods that are required on server only. Can be installed for pure singleplayer performance.

| Server Mods                                   | Status                      |
| --------------------------------------------- | --------------------------- |
| Achievements Optimizer                        |                             |
| AI Improvements                               |                             |
| Concurrent Chunk Management Engine (NeoForge) |                             |
| Create: Threaded Trains                       |                             |
| Disconnect Packet Fix                         |                             |
| Does It Tick?                                 |                             |
| FallingTree                                   |                             |
| Fast Noise                                    | Added Next Version 1.1.11   |
| FerriteCore                                   |                             |
| Get It Together, Drops!                       |                             |
| Ksyxis                                        | Removed Next Version 1.1.11 |
| Lithium                                       |                             |
| ModernFix                                     |                             |
| Mods Optimizer                                |                             |
| Packet Fixer                                  |                             |
| Redirected                                    |                             |
| ScalableLux                                   |                             |
| Server Core                                   |                             |
| spark                                         |                             |
| Structure Layout Optimizer                    |                             |

---

# SHARED LIBRARY MODS
## Shared Library Mods
Common library mods used by all of the above mods.

| Shared Mods                    | Status                      |
| ------------------------------ | --------------------------- |
| BagguetteLib                   | Removed Next Version 1.1.11 |
| Architectury API               |                             |
| Balm                           |                             |
| Bookshelf                      |                             |
| Chunk Activity Tracker         |                             |
| Cloth Config API               | Removed Next Version 1.1.11 |
| Cobblemon Tim Core             |                             |
| CreativeCore                   |                             |
| Cristel Lib                    |                             |
| Fzzy Config                    |                             |
| Geckolib                       |                             |
| GlitchCore                     |                             |
| Konkrete                       |                             |
| Kotlin for Forge               |                             |
| Melody                         |                             |
| Moonlight Lib                  |                             |
| oωo (owo-lib)                  |                             |
| Prickle                        |                             |
| Puzzles Lib                    |                             |
| Radical Cobblemon Trainers API |                             |
| Resourceful Config             |                             |
| ShatterLib \| OctoLib          |                             |
| TerraBlender                   |                             |
| TxniLib                        |                             |
| YetAnotherConfigLib            | Removed Next Version 1.1.11 |
