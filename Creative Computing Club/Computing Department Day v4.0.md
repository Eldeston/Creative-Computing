# Main Schedule
## Day 1 (February 11): Schedule

| Time        | Activity                                        |
| ----------- | ----------------------------------------------- |
| 2:00 – 2:15 | Opening Program & Orientation                   |
| 2:15 – 3:00 | Game 1: Cyber Lockdown Escape                   |
| 3:00 – 3:40 | Game 2: Password Relay Challenge                |
| 3:40 – 4:00 | Short Break                                     |
| 4:00 – 4:30 | Game 3: Digital Forensics Hunt                  |
| 4:30 – 5:00 | Robot Making & Design Challenge (Open Activity) |
## Day 2 (February 12): Schedule

| Time        | Activity                               |
| ----------- | -------------------------------------- |
| 2:00 – 2:15 | Opening & Rules Briefing               |
| 2:15 – 3:15 | Call of Duty Scrimmage                 |
| 3:15 – 3:30 | Short Break                            |
| 3:30 – 4:30 | Debug Dome: Python Debugging Challenge |
| 4:30 – 5:00 | Final Awards & Closing Program         |
# Department Day Marshals
List of games to manage:
- Cyber Lockdown
- Password Relay
- Digital Forensics

| Officers            | Role              | Course                | Managing                         |
| ------------------- | ----------------- | --------------------- | -------------------------------- |
| Frem Patalinghug    | President         | Creative Computing L5 |                                  |
| Reyn Santos         | President         | Cybersecurity L5      |                                  |
| Dexter Hingada      | Vice President    | Creative Computing L4 |                                  |
| Eshan Patel         | Vice President    | Cybersecurity L4      |                                  |
| Gayl Ragunjan       | Secretary         | Creative Computing L4 |                                  |
| John Yumul          | Secretary         | Cybersecurity L4      |                                  |
| Vince Caballero     | Event Coordinator | Creative Computing L5 | Cyber Lockdown                   |
| Rameen Akram        | Event Coordinator | Cybersecurity L5      | Password Relay/Digital Forensics |
| Lavhinia Joice      | Event Coordinator | Cybersecurity L5      | Password Relay                   |
| Rida Rashid         | Creative Director | Creative Computing L5 | Host                             |
| Maudric Patalinghug | Creative Director | Creative Computing L5 |                                  |
| Lanz Austria        | Creative Director | Cybersecurity L4      |                                  |
| Jan Ricalde         | Creative Director | Cybersecurity L4      |                                  |

| Judges             | Role       |
| ------------------ | ---------- |
| Mr. Oliver Malapit | Debug Dome |
| Ms. Lavanya Mohan  | Debug Dome |
# Registration Form (Secretaries)
Forms should give the participants options to register one of the 6 activities for Computing Department Day.

Separate Registration Forms for:
- [ ] Day 1 Activities - Open January 27th - Deadline February 11th
- [ ] Day 1 Robot Building - Open January 27th - Deadline February 9th
- [ ] Day 2 Scrimmage (IFS) - Open January 27th - Deadline February 11th
- [ ] Day 2 Debug Dome - Open January 27th - Deadline February 12th
# Poster Graphics (Creative Directors)
- [x] Main Poster - Instagram
![](Frame%201.png)
- [x] Main Poster - Landscape 16:9
![](Landscape%2016_9.png)
# Certificate Graphics (Creative Directors)
- [x] Certificate of Recognition
- [x] Certificate of Participation
# Decorations & Events Setup (Event Coordinators & Club Moderators)
To enhance the overall atmosphere and visual impact of CYBERSPARK 2026, thematic decorations will be prepared to reflect the event’s focus on computing, cybersecurity, innovation, and gaming. These decorations will help create an engaging environment and reinforce the event’s identity.

**Decoration Theme**
- [ ] Hackathon Theme
- [ ] Cybersecurity & Technology
- [ ] Gaming and e-sports elements

**Decoration Inclusions**
- [ ] Event signage (Game 1, Game 2, Robotics Area, COD Zone)
- [ ] Landscape with QR codes for Computing Department Day
- [ ] Table labels and directional signs

**Decoration Areas**
- [ ] Debug Dome stations
- [ ] Main Room/Opening Program area
- [ ] Robot Making & Design Exhibit area
- [ ] Call of Duty Scrimmage gaming zone
# Reward Specifications

| Medals | Trophies | Certificate of Recognition | Certificate of Participation |
| ------ | -------- | -------------------------- | ---------------------------- |
| 29     | 5        | 24                         | 18                           |

| Activities        | Rewards                                                          |
| ----------------- | ---------------------------------------------------------------- |
| Cyber Lockdown    | 5 Medals, 5 Certificate of Recognition                           |
| Password Relay    | 5 Medals, 5 Certificate of Recognition                           |
| Digital Forensics | 5 Medals, 5 Certificate of Recognition                           |
| CODM Scrimmage    | 1 Trophy, 5 medals, 5 Certificate of Recognition                 |
| Debug Dome        | 2 Trophy, 1 Gift Card, 11 Certificate of Participation           |
| Robot Making      | 2 Trophy, 1 Gift Card, 11 Certificate of Participation, 9 Medals |
## Medal Sticker Specifications
**Size:**
4 Centimeters in diameter

**Material:**
Gloss paper
Matte paper sticker
## Trophy Sticker Specifications
**Size:**
4 Centimeters in width
3 Centimeters in height

**Material:**
Gloss paper
Matte paper sticker
# Day 1 Activities
Team Count: 4
Winner: 1 Team
Team Members: 5
Rewards : 15 medals (5 each game, 1 each member), Certificate of Recognition
## Cyber Lockdown: Starts 2:15PM - Ends 3:00PM (45 Minutes)
**Description:**
Teams solve cybersecurity-themed puzzles to “escape” a simulated cyber attack.

**Mechanics:**
- Correct answers unlock the next clue
- The first teams to complete all puzzles win
- Each team receives clues related to phishing, malware, encryption, and safe browsing

**Materials Needed:**
- Timer
- Whiteboard
- Envelopes with clues
- Printed puzzle sheets

**Manpower:**
- 2 Game Marshals
- 1 Timekeeper
## Password Relay: Starts 3:00PM - Ends 3:40 (40 Minutes)
**Description:**
A physical and mental relay game focusing on password security and best practices.

**Mechanics:**
- Each team member completes a short physical task
- After each task, the player answers a password-related question
- Incorrect answers send the team back to repeat the task

**Materials Needed:**
- Cones
- Password question cards
- Whistle

**Manpower:**
- 2 Facilitators
- 1 Scorekeeper
## Digital Forensics: Starts 4:00PM - Ends 4:30PM (30 Minutes)
**Description:**
A scavenger hunt where teams investigate a simulated cybercrime.

**Mechanics:**
- Teams search for hidden QR codes around the venue
- Each QR code reveals a digital clue
- Teams analyze clues to identify the “hacker” and explain the incident

**Materials Needed:**
- Printed QR codes
- Smartphones (students’ own)
- Answer sheets

**Manpower:**
- 3 Facilitators
- 1 Technical Assistant
## Robot Making (Arts & Crafts): 4:30PM - Ends 5:00PM (30 Minutes)
Participants: Individual or group (maximum 3 members)
Rewards: 3 trophies, 1 Gift Card (AED 20), Certificate of Recognition, Certificate of Participation

**Description:**
An open creative activity for students passionate about innovation, design, and robotics. Participants will design and build a robot prototype or concept that solves a real-world problem. Announce the competition 2 weeks prior to participants for preparations. Participants will buy their own materials.

**Format:**
- Open to all students (individual or group)
- Designs may be:
- Physical models (recycled materials, basic kits)
- Concept boards, sketches, labeled diagrams
### Output & Display
- All robot designs will be displayed along school hallways as a Technology & Innovation Exhibit
### Scoring System

| Criteria                     | Points |
| ---------------------------- | ------ |
| Creativity and originality   | 40 pts |
| Design and functionality     | 20 pts |
| Problem-solving concept      | 20 pts |
| Presentation and explanation | 20 pts |

**Awards:**
- 3 Official Winners
- Cash prize OR
- Additional academic marks (subject to approval)
# Day 2 Activities
## Scrimmage - Call of Duty (IFS): 2:15PM - Ends 3:15PM (1 Hour)
Team Count: 4
Team Members: 5
Prizes: 5 Medals, 1 Trophy, Certificate of Participation

**Description:**  
The call of duty Scrim is a friendly yet competitive tournament among the players. This scrim aims to strengthen camaraderie, promote teamwork, and showcase members’ gaming skills in a fair and exciting competition.

**Game Mode:**  
Each match will be played in Call of Duty: Mobile – Search and Destroy (S&D). 

**Qualified players:**
Open to all Metaverse Computing, Cybersecurity, and IFS students across all levels. However, only 20 players will be selected to compete in the main event.
### Format
**Elimination Round**
- Round 1: Group 1 vs Group 2
- Round 2: Group 3 vs Group 4

**Finals**
- Round 3: Winner of Round 1 vs Winner of Round 2

**Rules & Regulations**
1. No cheating, exploiting, or use of unauthorized third-party software
2. Toxicity or unsportsmanlike behavior will lead to disqualification
3. All matches must be played live with no replays unless technical issues occur
4. Team captains must report match results immediately to the event coordinator
### Awards 
- 1 team official winner ( 5 players ) 
- 5 certificates 
- 1 trophy
## Debug Dome - Python Cyber Debug Challenge: 3:30PM - Ends 4:30PM (1 Hour)
Participants: Individuals (maximum 15 individuals), Creative Computing, Cybersecurity
Prizes: 1 Trophies, 1 Gift Card (AED 20), Certificate of Participation

Debug Dome is a cybersecurity-themed Python debugging competition where students act as Cyber Defense Engineers tasked with fixing vulnerable and broken Python programs before a simulated cyber attack causes system failure.

Instead of writing code from scratch, participants must analyze, debug, and secure existing Python programs, making the challenge accessible to both 1st and 2nd year students while still testing logical thinking, syntax knowledge, and cybersecurity awareness.
### Objectives
The competition aims to:
- Develop debugging and problem-solving skills
- Strengthen understanding of Python fundamentals
- Introduce secure coding practices
- Enhance teamwork and analytical thinking
- Simulate real-world cybersecurity scenarios
### Competition Format
- Type: Individuals
- Language Used: Python
- Duration: 40 minutes
- Difficulty Level: Progressive (Easy → Medium → Security-focused)
### Scoring System

| Criteria               | Points  |
| ---------------------- | ------- |
| Syntax Bug Fixed       | 10 pts  |
| Logic Bug Fixed        | 15 pts  |
| Security Improvement   | 20 pts  |
| Code Runs Successfully | 20 pts  |
| Clean & Readable Code  | 10 pts  |
| Total Possible         | 100 pts |
Time bonus: +10 pts for early completion

### Materials & Requirements
**For Participants:**
- Laptop with Python installed
- Code editor (VS Code / IDLE / PyCharm)

**For Organizers:**
- Printed problem sheets
- USB / Google Drive access to files
- Timer
- Rubrics
# Final Budget (Secretaries)

| Available Inventory | Count |
| ------------------- | ----- |
| Medals              | 25    |
| Certificates        | ∞     |

| Items                               | Budget  |
| ----------------------------------- | ------- |
| 5 Trophies                          | AED 50  |
| Hanging Decorations                 | AED 29  |
| 2 Digital Steam Gift Cards (20 AED) | AED 36  |
| Total                               | AED 115 |
